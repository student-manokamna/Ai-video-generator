...
export function SignupFormDemo() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const { data, error } = await authClient.signUp.email({
            name: firstname.current?.value!, // required
            email: email.current?.value!, // required
            password: password.current?.value!, // required,
            fetchOptions:{
                onSuccess:()=>{
                    router.push("/login")
                },
    ...
    return (
        <div className="shadow-input mx-auto w-full max-w-md rounded-none bg-white p-4 md:rounded-2xl md:p-8 dark:bg-black">
            ...
            <h2 className="mt-2 max-w-sm text-sm text-neutral-600 dark:text-neutral-300">
            ...
            <form className="my-8" onSubmit={handleSubmit}>
                ...
                <LabelInputContainer className="mb-4">
                    ...
                    <Input id="password" placeholder="••••••••" type="password" ref={password} minLength={8} />
                </LabelInputContainer>
